...
{
	int		Frame;	// 몇번째 프레임에 등록할 것인가
	bool	Call;	// 현재 모션에서 call되었는지
...
/*
애니메이션 시퀀스를 하나 가지고 있으면서 해당 애니메이션을 총 몇초에
걸쳐서 한번 재생을 할것인지, 이 애니메이션이 계속 반복해서 재생하는지
등을 정의해두는 객체이다.
*/
...
private:
	//해당 Info는 어떤 애니메이션 시퀀스를 가지고 있으면서 애니메이션을 돌려줄 Info인가
	...
	float		m_PlayTime;		// 모션 한번이 동작 되기 위한 시간.
