...
void CCamera::Update(float DeltaTime)
{
	...
	else if (m_CameraShakeOn)
	{
		// 카메라가 월드를 벗어나지 못하도록 만들어준다.
		if (m_Pos.x < -30.f)
			m_Pos.x = -30.f;

		else if (m_Pos.x + m_Resolution.x > m_WorldResolution.x + 30.f)
			m_Pos.x = m_WorldResolution.x - m_Resolution.x + 30.f;

		if (m_Pos.y < -30.f)
			m_Pos.y = -30.f;

		else if (m_Pos.y + m_Resolution.y > m_WorldResolution.y + 30.f)
			m_Pos.y = m_WorldResolution.y - m_Resolution.y + 30.f;
